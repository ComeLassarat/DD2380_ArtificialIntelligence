...
(:action have-lunch 
    :parameters (?x ?y)
    :precondition (and (PERSON ?x) (BUILDING ?y) (HAS-RESTURANT ?y) (is-person-at ?x ?y) (not (had-lunch ?x)))
    :effect (and (had-lunch ?x) (is-person-at ?x ?y))
...
(:action attend-morning-lecture
    ...
    :precondition (and 
                ...
                (IS-LECTURE-AT ?w ?z)
                (teaches-lecture ?y ?w))
    :effect (and (attended-lecture ?x ?w) (is-person-at ?x ?z) (is-person-at ?y ?z))
	; WRITE HERE THE CODE FOR THIS ACTION
)
