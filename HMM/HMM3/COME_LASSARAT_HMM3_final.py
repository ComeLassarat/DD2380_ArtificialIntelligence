...
def re_estimate(matrix_A, matrix_B, matrix_pi, emissions):
...
########################## ESTIMATION ##############################
    ...
    return matrix_A,matrix_B,matrix_pi,C
...
matrix_A, matrix_B, matrix_pi, C = re_estimate(matrix_A_extracted, matrix_B_extracted, matrix_pi_extracted, emissions_extracted)
iters = 1
logProb = 0
maxIters = 1000
...
while (iters < maxIters and logProb > oldLogProb and time.time() - start_time < 0.8):
    oldLogProb = logProb
    matrix_A,matrix_B,matrix_pi,C = re_estimate(matrix_A, matrix_B, matrix_pi, emissions_extracted)
    for i in range(nb_of_emissions):
        logProb = logProb + math.log(C[i])
    logProb = -logProb
    iters+=1
