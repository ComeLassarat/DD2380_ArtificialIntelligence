...
class FishingDerbyRLApp(App, SettingLoader, Communicator, PrintScore1Player):
    ...
    def init_states(self):
        ...
        for i in range(subdivisions):
            for j in range(subdivisions):
                state2ind[state] = (i, j)   #Dict of the different positions on the grid map, which correponds to the different states : INDEX TO STATE
                ind2state[(i, j)] = state   #Dict of the different states, which correponds to the position on the grid map : STATE TO INDEX
                state += 1
    ...
    def init_actions(self):
        ...
        self.actions2ind = {
            "left": 0,
            "right": 1,         #Index of the different actions
            "down": 2,
            "up": 3,
            "stay": 4
        }
    ...
    def step(self, action):
        ...
        if not self.player.diver.model[ind_state, ind_action]:   #???
            ...
            next_state = next_state_x, next_state_y                     #We compute the next state
