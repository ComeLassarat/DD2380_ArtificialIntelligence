...
class PlayerControllerRL(PlayerController, FishesModelling):
    ...
    def q_learning(self):
        ...
        for s in range(ns):
            ...
            for i in range(4):
                if i not in list_pos:       #if this is not an allowed move
                    Q[s, i] = np.nan           #nan = "not a number" --> replace with this because to hard to manage nan
        ...
        init_pos_tuple = self.settings.init_pos_diver                       #initial pose of the diver
        init_pos = self.ind2state[(init_pos_tuple[0], init_pos_tuple[1])]   #initial state of the diver. State = a number
        episode = 0
        ...
        while episode <= self.episode_max and diff > self.threshold:
            ...
            while not end_episode:
                ...
                action = np.nanargmax(line)   #We take the action maximizing Q[s_current][:]
                #if action not in list_pos:
                    #action = np.ra
                #print(action)
