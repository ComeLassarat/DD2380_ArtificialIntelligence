...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':              #epsilon = constant
        ...
        if random_nb < 1-epsilon:           #with a probability of 1-epsilon :
            line = Q[state][:]
            action = np.nanargmax(line)     #take argmax Q (exploitation)
        else :                              #with a probability of epsilon
            action = all_actions[np.random.randint(len(all_actions))]       #take a random action (exploration)
        ...
        # It is recommended you use the np.random module
        random_nb = np.random.random()
        schedule = ScheduleLinear(anneal_timesteps, epsilon_final, initial_p=1.0)  #
        epsilon_t = schedule.value(current_total_steps)     #we compute epsilon_t
        
        
        if random_nb < epsilon_t:               #same as for eps_type = 'constant'
            action = all_actions[np.random.randint(len(all_actions))]
        else:
            line = Q[state][:]
            action = np.nanargmax(line)
        # ADD YOUR CODE SNIPPET BETWEENEX  3.2
    else:
    ...
    def q_learning(self):
        ...
        while episode <= self.episode_max and diff > self.threshold:
            ...
            while not end_episode:
                ...
                if current_total_steps < 10000:
                    
                    action = epsilon_greedy(Q,s_current,list_pos, current_total_steps = current_total_steps ,epsilon_initial=1, epsilon_final=0.2, anneal_timesteps=10000, eps_type="linear")
                else:
                    action = epsilon_greedy(Q,s_current,list_pos,current_total_steps = 10000,
                    epsilon_initial=1,
                    epsilon_final=0.2,
                    anneal_timesteps=10000,
                    eps_type="constant")                            #We reached epsilon_final
                # ADD YOUR CODE SNIPPET BETWEEN EX 2.1 and 2.2
    ...
    def random_agent(self):
        ...
        while episode <= self.episode_max:
            ...
            while not end_episode:
                ...
                action = possible_actions[np.random.randint(len(possible_actions))]   #Picking a random possible action given the current state
                n[s_current,action] +=1                                               #Adding the occurence of this action to the matrix n, given the current state
